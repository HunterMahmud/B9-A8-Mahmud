...
const  BooksCard = ({ book, path }) => {
  ...
  return (
    <Link to={`/${path}/${bookId}`} className={`card ${book?.ageRange?"bg-pink-400":"bg-base-100"} border w-[90%] sm:max-w-[400px] mx-auto border-gray-300`}>
      ...
      <div className="card-body items-start">
        <div className="flex gap-3">
            {
                tags.map((tag,i)=> <p className={`${book?.ageRange?"text-pink-700 bg-pink-200":"text-[#23BE0A] bg-[#23BE0A0D]"} font-medium  py-2 px-3 rounded-[30px]`} key={i}>{tag}</p>)
            }
        </div>
